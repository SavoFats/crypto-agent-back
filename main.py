...
def quantize_str(value: float, increment: Optional[str], rounding) -> str:
    if not increment:
...
async def build_buy_order_config(product_id: str, size: float, price: float) -> Tuple[Optional[Dict], Optional[str]]:
    product = await fetch_coinbase_product(product_id)
    if (
        product.get("error")
        or product.get("trading_disabled")
        or product.get("is_disabled")
        or product.get("cancel_only")
    ):
...
async def build_sell_order_config(product_id: str, qty_coin: float, price: float) -> Tuple[Optional[Dict], Optional[str]]:
    product = await fetch_coinbase_product(product_id)
    if (
        product.get("error")
        or product.get("trading_disabled")
...
async def enter_position(sym_data: dict):
    ...
    if is_real:
        try:
            product_id = f"{sym}-{QUOTE_CURRENCY}"
            ...
            if result.get("success") != True:
                ...
                if "limit only mode" in err_lower:
                    product_cache.pop(product_id, None)
                    cooldown_symbol(sym, f"{product_id} accetta solo limit order")

                if "account is not available" in err_lower:
                    cooldown_symbol(sym, f"{product_id} account non disponibile")
            ...
            add_log(
                "buy",
                "ACQUISTO REALE",
                f"{sym} @ ${actual_price:.4f} | Size: ${size:.0f} | SL: {sl_pct*100:.1f}% | Trailing ON",
            )
        except Exception as e:
...
async def exit_position(pos: dict, reason: str):
    ...
    if pos.get("realMode", False):
        try:
            qty_coin = pos["size"] / pos["entryPrice"]
            product_id = f"{sym}-{QUOTE_CURRENCY}"
...
async def scan_and_trade():
    ...
    btc_ok = btc_1h >= -0.3
    if not btc_ok:
        add_log("info", "PAUSA", f"BTC {btc_1h:+.2f}% 1h — agente in attesa")
        _update_pnl()
        return
    ...
    candidates = [
        d for d in ranked
        if d["change24h"] >= min_mom
        and d.get("change1h", 0) > 0
    ]
...
async def start_agent(body: dict):
    ...
    agent_state.update({
        ...
        "config": {
            ...
            "orderErrorCooldownMin": float(cfg.get("orderErrorCooldownMin", 30)),
        },
        "cooldowns": {},
        "tradeCount": 0,
        "wins": 0,
    ...
    add_log(
        "info",
        "AVVIO",
        f"${capital:.0f} | {mode} | Alloc: {alloc:.0f}% | SL: {sl:.1f}% | Vol: ${vol:.0f}M | Quote: {QUOTE_CURRENCY}",
    )
    return {"ok": True}
